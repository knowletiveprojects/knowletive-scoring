...
def today_leaderboard(batch_id: int = None, db: Session = Depends(get_db)):
    today = date.today()
    query = (
...
def weekly_leaderboard(batch_id: int = None, db: Session = Depends(get_db)):
    today = date.today()
    ...
    if batch_id is not None:
        query = query.filter(Student.batch_id == batch_id)
    results = query.group_by(Student.id, Student.name).order_by(func.sum(Score.total).desc()).limit(20).all()
    return [{"name": r.name, "student_id": r.student_id, "total": r.total, "rank": i + 1}
            for i, r in enumerate(results)]


@router.get("/leaderboard/monthly", response_model=List[dict])
def monthly_leaderboard(batch_id: int = None, db: Session = Depends(get_db)):
    today = date.today()
...
def student_of_the_day(batch_id: int = None, db: Session = Depends(get_db)):
    today = date.today()
    query = (
...
def all_students_average(days: int = 7, batch_id: int = None, db: Session = Depends(get_db)):
    end = date.today()
    ...
    if batch_id is not None:
        query = query.filter(Student.batch_id == batch_id)
    results = query.group_by(Student.id, Student.name, Student.email, Student.level).order_by(func.avg(Score.total).desc().nulls_last()).all()
...
def get_all_streaks(batch_id: int = None, db: Session = Depends(get_db)):
